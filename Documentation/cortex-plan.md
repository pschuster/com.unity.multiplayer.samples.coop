...
| **3** | Baustein 5 + Baustein 4 Schichten 1–2 + Eskalationsleiter | A1, A4, B3, B4 | ~2 Wochen | 0, 2; Node-SDK-Fix |
...
|---|---|---|---|
| Node-SDK | `setChannelMasks` und `channelMasks` (Join): Masken als `[[peerId, mask], …]` serialisieren | Bot-Mute, Baustein 4.2 | **MR gestellt:** [odin-node-js!3](https://gitlab.4players.de/odin/tools/odin-node-js/-/merge_requests/3), gegen Live-Gateway verifiziert. Stand 2026-09-22: `@4players/odin-nodejs@2.0.1` liegt noch **nicht** auf npm (`next` → 2.0.0); der Cortex-Bump ist inzwischen gemerged und deployt: [odin-transcription-service!12](https://gitlab.4players.de/odin/integrations/odin-transcription-service/-/merge_requests/12) (2.0.1, `channelMasks` beim Join, `setPeerAudible` über `setChannelMasks`) |
...
| Payment | `create-token` um `tags`, `lifetime` erweitern | Join-Gate und Bot-Tag beim `rooms`-Provider | offen |
...
- Nach Phase 3: Polling-Loop raus; `OnMessageReceived`-Listener mit Absenderprüfung, Mute über
  `Room.SetChannelMasks`. Das ist dann die Referenzimplementierung.
